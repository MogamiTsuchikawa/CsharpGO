...
namespace MainEditor
{
    ...
    "using", "virtual", "void", "volatile", "while",//"chomado","xamarin","( *ﾟ▽ﾟ*  っ)З ぽぽー！","( *ﾟ▽ﾟ*  っ)З ","(((o(*ﾟ▽ﾟ*っ)っ-=三",
}, CharClass.Keyword);
    ...
    "#region", "#undef", "#warning"
            ...
            if (files.Length != 0)
            {
                for (int i = 0; i < files.Length; ++i)
                {
                    ...
                    {
                        if (System.IO.Path.GetExtension(files[i]) != ".guit")
                        {
    ...
    "#region", "#undef", "#warning"
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            var files = System.IO.Directory.GetFiles(Hensu.ProjectPath + @"\source", "*", System.IO.SearchOption.AllDirectories);
            files = files.Where(x => x != Hensu.ProjectPath + @"\source\main.cs").ToArray();
            if (System.IO.Path.GetExtension(e.Node.Text) == ".guit")
            {
                //デザイナー起動
