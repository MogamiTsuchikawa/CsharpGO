...
namespace MainEditor
{
    class Build
    {
        ...
        public static string consoleBuild()
        {
            var projpath = Hensu.ProjectPath;
            var dfiles = Directory.GetFiles(projpath + @"\build");
            int i = 0;
        ...
        static string MSbuild()
        {
            ...
            p.StartInfo.Arguments = "/c cd /d" + Hensu.ProjectPath + @"\build" + @"&& C:\Windows\Microsoft.NET\Framework\v4.0.30319\MSBuild.exe";
            ...
            p.WaitForExit();
            var rt = p.StandardOutput.ReadToEnd();
            p.Close();
            return rt;
        }
    }
